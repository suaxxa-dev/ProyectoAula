(function () {
        ...
        try {
            ...
            products.forEach(p => {
                ...
                if (catKey) {
                    ...
                    if (catKey === 'accesorios' && !secs.includes('accesorios')) secs.push('accesorios');
                    if (catKey === 'deportes' && !secs.includes('deportes')) secs.push('deportes');
                    ...
                    CATALOG[catKey].push({
                        ...
                        sku: p.sku,
                        stock: p.stock || 0,
                        temas: (p.temas || '').split(',').map(s => s.trim()).filter(Boolean),
                        secciones: secs 
                    });
                }
            });
    ...
    function filterProducts(list) {
        ...
        return list.filter(function (p) {
            ...
            if (hasMarcaFilter) {
                var matchMarca = false;
                if (selectedMarcas.indexOf(p.marca) !== -1) {
                    matchMarca = true;
                } else if (selectedMarcas.indexOf('Otros') !== -1) {
                    var catBrands = BRANDS_BY_CATEGORY[p._catKey] || [];
                    if (!p.marca || catBrands.indexOf(p.marca) === -1) {
                        matchMarca = true;
                    }
                }
