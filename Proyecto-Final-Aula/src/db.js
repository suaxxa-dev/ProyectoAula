...
async function initDb() {
  ...
  // Migración: Asegurar que la columna 'temas' exista
  try {
    db.run("ALTER TABLE productos ADD COLUMN temas TEXT DEFAULT ''");
  } catch (e) {}

  // Migración: Asegurar que la columna 'genero' exista
  try {
    db.run("ALTER TABLE productos ADD COLUMN genero TEXT DEFAULT ''");
  } catch (e) {}
...
function createProduct(p) {
  ...
  const pTemas = p.temas !== undefined ? p.temas : '';
  const pGenero = p.genero !== undefined ? p.genero : '';

  try {
    runSql(`
      INSERT INTO productos (id, nombre, precio_num, precio_label, img, stars, desc_pct, promo_badge, marca, tipo, cat, sku, stock, temas, genero)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [p.id, p.nombre, pNum, pLabel, p.img, p.stars || 5, dPct, pBadge, p.marca || '', p.tipo || 'nuevo', p.cat, p.sku, p.stock || 0, pTemas, pGenero]);
    return { ok: true };
  } catch (err) {
...
function updateProduct(id, p, adminId) {
  ...
  const sets = [];
  const values = [];
  const fields = ['nombre', 'precioNum', 'precioLabel', 'img', 'stars', 'descPct', 'promoBadge', 'marca', 'tipo', 'cat', 'sku', 'stock', 'temas', 'genero'];
  
  fields.forEach(f => {
    let dbField = f;
    let val = p[f];
    // Mapear camelCase a snake_case y aceptar ambos formatos
    if (f === 'precioNum') { dbField = 'precio_num'; if (val === undefined) val = p.precio_num; }
    if (f === 'precioLabel') { dbField = 'precio_label'; if (val === undefined) val = p.precio_label; }
    if (f === 'descPct') { dbField = 'desc_pct'; if (val === undefined) val = p.desc_pct; }
    if (f === 'promoBadge') { dbField = 'promo_badge'; if (val === undefined) val = p.promo_badge; }
